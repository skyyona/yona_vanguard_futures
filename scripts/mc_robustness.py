"""Canonical MonteCarlo robustness pilot (single clean copy).

This file is the production canonical script used by orchestration.
"""
...
def parse_args():
    ...
    p.add_argument('--slippage-grid', type=str, default='',
                   help='Comma-separated slippage values to sweep, e.g. "0.0,0.0005,0.001"')
    p.add_argument('--max-fee', type=float, default=0.0005)
    p.add_argument('--noise-sigma', type=float, default=0.001)
    p.add_argument('--enable-vol-filter', action='store_true', dest='enable_vol_filter', help='Enable volume-spike entry filter')
    p.add_argument('--vol-filter-threshold', type=float, default=5.0, dest='vol_filter_threshold', help='Volume multiplier threshold to block entries')
    p.add_argument('--vol-filter-lookback', type=int, default=10, dest='vol_filter_lookback', help='Lookback for rolling median volume')
    return p.parse_args()
...
def main():
    ...
    tasks = []
    seed_base = 12345
    # parse slippage grid if provided (overrides random slippage sampling)
    slippage_grid = None
    if args.slippage_grid:
        try:
            slippage_grid = [float(s) for s in args.slippage_grid.split(',') if s.strip()]
        except Exception:
            raise SystemExit('Invalid --slippage-grid format; expected comma-separated floats')

    for idx, row in enumerate(candidates, start=1):
        params_base = parse_params_from_wf_row(row)
        # apply optional runtime volume filter settings
        if args.enable_vol_filter:
            params_base['enable_volume_spike_filter'] = True
            params_base['vol_spike_threshold'] = float(args.vol_filter_threshold)
            params_base['vol_spike_lookback'] = int(args.vol_filter_lookback)
        if slippage_grid:
            # sweep provided slippage values; keep fee randomized per iteration
            for sl in slippage_grid:
                for it in range(args.mc_iter):
                    fee = random.uniform(0.0, args.max_fee)
                    seed = seed_base + idx * 1000 + it + int(sl * 1000000)
                    tasks.append((symbol, args.interval, args.start, args.end, params_base, sl, fee, args.noise_sigma, seed, idx))
        else:
            for it in range(args.mc_iter):
    ...
    fieldnames = ['rank','stop_loss_pct','position_size','take_profit_pct','volume_spike_factor','slippage_pct','mc_mean_profit_pct','mc_std_profit_pct','mc_5th_pct','mc_negative_pct','mc_samples']
