...
SYMBOLS = ['JELLYJELLYUSDT','BTCUSDT','ETHUSDT','SOLUSDT','LTCUSDT','XRPUSDT']

async def run_for(symbol, interval='1m', days=1):
...
async def main():
    ...
    summary = []
    for s in SYMBOLS:
        res = await run_for(s, '1m', days=1)
        if res is None:
            continue
        # write JSON (use default=str to serialize timestamps)
        out_path = os.path.join(OUT_DIR, f"{s}.json")
