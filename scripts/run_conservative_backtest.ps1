$start = [int64][double]((Get-Date).ToUniversalTime().AddDays(-30) - (Get-Date '1970-01-01')).TotalMilliseconds
...
while ($true) {
    ...
    if (((Get-Date) - $startTime).TotalSeconds -gt 600) { Write-Host 'Timeout waiting for run to complete'; break }
}
