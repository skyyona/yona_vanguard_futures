"""Grid-run driver: execute next_open_masked_sim.py over parameter grid and date range,
collect per-day summaries and aggregate into a single CSV for analysis.

Usage (example):
  python .\scripts\grid_run_next_open.py
"""
...
def main():
    ...
    dates = [d.strftime('%Y-%%m-%d') for d in [dt for dt in daterange(start,end)]]
    ...
    out_path = os.path.join('results', f"pippinusdt_next_open_grid_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.csv")
    if out_rows:
