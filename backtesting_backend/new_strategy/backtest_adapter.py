...
class BacktestExecutor:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for idx in range(total_candles):
            ...
            # Orchestrator step 실행 (안전 호출)
            try:
                try:
                    result = self.orchestrator.step()
                except RuntimeError:
                    # 발생 원인: 현재 스레드에 이벤트 루프가 실행 중인 경우
                    # 안전하게 비동기 코루틴을 스케줄링하여 동기적으로 결과를 기다립니다.
                    from backend.core.async_utils import schedule_coro_sync
                    result = schedule_coro_sync(
                        self.orchestrator.step_async(),
                        target_loop=getattr(self.orchestrator, "_thread_loop", None)
                    )
